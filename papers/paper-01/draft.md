...
**Optional Enhancement Layers.** The Monte Carlo engine supports three optional enhancement layers that extend the base competing risks simulation. First, *organ acceptance rate modeling* applies a parametric thinned Poisson process with per-center volume-derived acceptance factors, calibrated from SRTR center-level transplant volumes, to model the probability that a given organ offer is accepted by a center. Second, *dynamic MELD/LAS score progression* models acuity score drift over waitlist time using a piecewise-linear trajectory with interval-specific clinical multipliers derived from published MELD progression literature, capturing the clinical reality that sicker patients accrue priority more rapidly. Third, *population trend projection* extrapolates donor supply and candidate listing rates using linear regression on 2019--2025 SRTR historical data (15 biannual releases), enabling forward-looking simulations that account for secular trends in organ availability and waitlist growth. All three layers are optional and disabled by default; when enabled, they compose with the base simulation by modifying event rates within the existing competing risks framework.
...
**COMET.** The Comprehensive Organ Matching and Evaluation Tool (COMET) is the most directly comparable recent work [Egan et al., 2024]. COMET uses agent-based modeling, where individual patients and organs are simulated as autonomous agents with organ acceptance behavior learned from historical data. This approach is more realistic than TransPlan's parametric competing risks framework, particularly for modeling the sequential organ offer process. TransPlan now includes an optional organ acceptance rate layer using a parametric thinned Poisson process with per-center volume-derived factors, which narrows this gap, though COMET's agent-based acceptance modeling remains more realistic by capturing individual donor-candidate interaction dynamics. However, COMET is currently implemented only for lung transplantation, whereas TransPlan covers all six solid organ types. TransPlan's lighter parametric approach enables real-time web-based interaction, whereas agent-based simulation typically requires longer runtimes.

**LivSim.** LivSim is an open-source liver allocation simulator designed for policy analysis [Kilambi et al., 2018]. Like the SAMs, LivSim models the allocation process at the individual organ-offer level, providing higher mechanistic fidelity than TransPlan for liver-specific policy questions. TransPlan now includes an optional MELD/LAS score progression layer using piecewise-linear drift with interval-specific clinical multipliers, which narrows the gap with LivSim's dynamic MELD modeling, though LivSim's discrete-event approach is more mechanistically detailed in tracking individual candidate trajectories. TransPlan complements tools like LivSim by offering cross-organ comparison and integration of geographic, environmental, and equity dimensions not addressed by allocation-focused simulators.
