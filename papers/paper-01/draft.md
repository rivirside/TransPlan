...
Despite this variation, patients and their clinical teams have limited computational support for center selection. The SRTR Program-Specific Reports provide per-center outcome statistics but do not integrate patient-level clinical factors (blood type, urgency score, organ-specific acuity measures such as cPRA, MELD, or LAS) into personalized wait time forecasts. Existing decision aids are either proprietary, restricted to single organs, or lack the statistical rigor to provide uncertainty-quantified predictions [Tong et al., 2015]. No publicly available tool simultaneously models competing risks on the waitlist (transplant, mortality, delisting), accounts for spatial heterogeneity in donor supply and environmental health determinants, or allows patients and researchers to compare alternative UNOS allocation policies through simulation.
...
The primary existing resource for transplant center comparison is the SRTR Program-Specific Reports (srtr.org), which provide per-center outcome statistics stratified by organ type. TransPlan differs from the SRTR reports in several respects. First, TransPlan integrates patient-specific clinical factors (blood type, urgency, acuity scores) into personalized predictions rather than presenting population-level statistics. Second, TransPlan models competing risks explicitly, providing decomposed probability estimates for transplant, mortality, and delisting rather than single-endpoint survival curves. Third, TransPlan incorporates geographic and environmental data layers (air quality, cost of living, county health indicators) that are absent from the SRTR reports. Fourth, TransPlan supports counterfactual policy analysis through its scenario engine, enabling researchers to evaluate the projected impact of allocation policy changes. The SRTR reports remain the authoritative source for observed transplant outcomes, and TransPlan's predictions should be interpreted as model-derived estimates that complement, rather than replace, SRTR data.
...
TransPlan demonstrates that a multi-engine probabilistic approach to transplant center evaluation is technically feasible and can be deployed as an accessible, open-source web application. By combining Monte Carlo simulation with competing risks, Bayesian network inference, and MCMC hierarchical modeling, the system provides triangulated center rankings that are robust to individual model assumptions. The eight-category scoring algorithm, 24 spatial data layers, five policy simulations, and 48-profile equity audit offer a breadth of analysis not available in existing transplant decision tools. Internal validation confirms self-consistent calibration and concordant cross-engine rankings, while extensive automated testing (717 tests) provides engineering confidence in implementation correctness.
