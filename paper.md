...
- **Model Validation**: Cross-*engine* concordance between the three inference engines (Spearman rank correlation, top-5 overlap), Monte Carlo-vs-analytical internal-consistency checks (Brier score), and temporal stability analysis. These are self-consistency diagnostics, not validation against external patient outcomes (which awaits SRTR microdata access).
- **Centers**: Browse, search, and filter all 248 SRTR centers with per-center detail pages.

The three inference engines offer complementary trade-offs: Monte Carlo simulation (~80ms, 1000 iterations) provides the most detailed competing risks modeling with optional Clayton copula dependence [@nelsen2006]; the Bayesian Belief Network (~30ms) uses exact variable-elimination inference (a self-contained NumPy engine) for deterministic real-time results; and the MCMC hierarchical survival model (~500ms) provides honest posterior uncertainty via PyMC [@salvatier2016] with LKJ-Cholesky correlated random effects [@lewandowski2009].
