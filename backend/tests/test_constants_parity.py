...
The no-build-step architecture duplicates DEFAULT_WEIGHTS in scoring-constants.js and
...
CONSTANTS_JS = Path(__file__).resolve().parent.parent.parent / "scoring-constants.js"


def _parse_js_default_weights(text: str) -> dict[str, float]:
    block = re.search(r"const DEFAULT_WEIGHTS\s*=\s*\{([^}]*)\}", text)
    assert block, "DEFAULT_WEIGHTS object not found in scoring-constants.js"
    return {
        key: float(val)
...
def test_default_weights_match_between_js_and_python():
    js_weights = _parse_js_default_weights(CONSTANTS_JS.read_text())
