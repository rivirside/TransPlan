...
## Interactive: Kidney Wait Time Distribution by Blood Type

Toggle between PDF (probability density) and CDF (cumulative probability) views. Notice how O blood types (longest waits) shift the distribution right, while AB types (shortest waits) shift it left.

<DistributionCurve
  median={27.4}
  sigma={1.2}
  title="Kidney Wait Time Distribution by Blood Type"
  maxMonths={72}
...
During each Monte Carlo iteration, the engine samples a raw wait time from the log-normal distribution, applies the center-specific factor, and then applies any relevant clinical multiplier:

```python
# Sample raw wait time
t_raw = lognormal(mu, sigma)

# Apply center-specific factor
t_center = t_raw * center_factor

# Apply clinical multiplier
t_final = t_center * clinical_multiplier(cpra, meld, las)
```

### Time already served

A candidate who has already waited is not in the same position as one
listing today, so when `months_waiting` is supplied the sample is **left
truncated**: the engine draws the remaining wait from the distribution
*conditional* on the total wait exceeding the time already served, rather
than restarting the clock.

```python
# Remaining wait, given t0 months already served
t_remaining = sample(T_total - t0, given T_total > t0)
```

This uses the same log-normal, so no separate parameters are involved. Note
the competing-risk clocks (mortality, delisting) are exponential and
therefore memoryless, so they legitimately do restart — only the wait clock
carries history.
...
## Interactive: All Organs Compared

Each organ has a different national median wait time. Heart and lung have the shortest waits (highest acuity), while kidney has the longest.

<DistributionCurve
  median={27.4}
  sigma={1.2}
  title="Wait Time by Organ (O+ blood type, national median)"
  maxMonths={72}
  showCDF
  bloodTypeMultipliers={{
    "Kidney (27.4mo)": 1.0,
