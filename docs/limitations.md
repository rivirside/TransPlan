...
- **"Downgrade its weight" turned out to be inapplicable: it has no weight in any ranking.** `scoring.py` never imports `allocation_geography`, and no center ranking, probability, wait estimate or competing-risk figure depends on it. Its only consumers are `GET /spatial/allocation-circles` and `/spatial/distance-score`, rendered on the Explorer's Spatial Analysis tab — so it cannot skew a recommendation, but a bare number labelled "Competition" beside a "Composite" score reads as a validated finding about a patient's odds.
- **Superseded 2026-08-28 (#432/#433):** the Explorer composite no longer uses the circle proxy at all. Its competition component now comes from the OPO measure below, with `competition_basis` recording which was used and the circle figure retained only for comparison. The circle count is now display-only reference data. `backend/tests/test_competition_proxy_scope.py` still holds the scoring-path boundary, which is the claim that matters for rankings.
...
- **The exponent is FITTABLE, and the shipped value is contradicted for five of six organs (measured 2026-08-28).** This was carried as untestable because SRTR publishes 12-month outcomes and the model reports at 24. But Table B7 publishes the **same cohort at 6, 12 AND 18 months** (`SAL_*_U6/_U12/_U18`), which is enough to observe the hazard's shape — and shape is what the exponent encodes. Interval hazards fall **monotonically** for kidney, liver, heart, lung and intestine (heart 0.2258 → 0.0704 → 0.0423, a 5.3× decline); pancreas alone is flat (0.0407 / 0.0375 / 0.0420). Extrapolating the observed 12–18 hazard across 18–24 and solving S(24)=S(12)^α:

| organ | implied α | shipped |
|---|---|---|
| kidney | **1.700** | 2.0 |
| liver | **1.500** | 2.0 |
| heart | **1.286** | 2.0 |
...
| pancreas | 2.074 | 2.0 |
  ...
  **Pancreas is the control:** the one organ with a genuinely flat hazard is the one where the method returns α ≈ 2.07, recovering the shipped assumption exactly where it holds. That is what makes the other five credible rather than arithmetic artifact.

  Adopting the fitted values would cut displayed mortality by **20–42%** (kidney 0.0531 → 0.0427, heart 0.0415 → 0.0242) and raise pancreas 4%. See `docs/horizon-alpha-fit.md` and `scripts/run-horizon-alpha-fit.py`.
- **Deliberately not adopted, and the reason is asymmetry.** Every other correction in this sweep either removed an unevidenced penalty (#415) or widened a too-narrow interval (#420) — both err toward caution. This one moves a **mortality figure downward**, making centers look safer, and it rests on extrapolating the 12–18 hazard across an unobserved 18–24 window rather than on a 24-month observation. Nor can an automated gate settle it: the calibration metric correlates p24, which is algebraically invariant to α. Needs the clinician sanity-check #107 exists for. Same disposition as #297, which found the delisting multipliers had the wrong **sign** and still did not change them (#380). Adoption filed as **#427** with the fitted values and the implementation sketch.
...
- **Still not propagated, and now quantified:** WaitCategory-*timing* uncertainty is carried by neither engine. Measured 2026-08-28 as **L-097** — moving σ across the clamp band shifts kidney p24 by 0.0968, about **0.56× the width of the interval this entry fixed**. So the band is now honest about what it covers and still understates the total, most for kidney. #296 stays open for that piece.
...
- **What:** the Phase 2 simulator rebuild renamed the results containers (`.results-section` → `#sim-results-section`, `.results-panel` → `#sim-results-container`) and the `@media print` block never followed. Measured against the live DOM with results rendered: **20 of 24 selectors matched nothing**, including every positive rule.
- **The consequence:** the footer *"Generated by TransPlan — for informational purposes only. Not a substitute for medical advice."* is `.results-section::after`. With that selector dead, **it never printed.** A patient printing their center list to take to a care team got a page with no disclaimer.
- **Why it survived:** a dead *hide* rule is invisible — the element simply prints. A dead *show* rule or `::after` removes content silently, and almost nobody prints, so nobody reports it. No test covered the print path.
- **Fixed:** block rewritten against current markup; live re-verification gives 26 live selectors and 1 runtime-only (`.rt-detail-row`, present only when a row is expanded). The pagination *count* is now kept while its buttons are hidden, so a printout of page 1 says "Showing 1–25 of 233 centers · page 1 of 10" instead of presenting a quarter of the list as the whole.
- **Guarded:** `__tests__/print-stylesheet.test.js` asserts every print selector matches `simulator.html` or sits on a runtime allowlist that names the module creating it. Negative-tested by renaming `#sim-results-section` — the exact rebuild rename that caused this. The test caught three errors in the first draft of the fix.
- **Generalizes:** any stylesheet keyed to markup can rot this way, and `@media print` is the case with no feedback loop. Worth asking of `@media` blocks generally: is anything checking these selectors still resolve?
- **Files:** `styles.css` (`@media print`), `__tests__/print-stylesheet.test.js`
...
- **Category:** Clinical Assumption / Statistical Validity
- **What:** the wait-time multipliers and the compatibility score both give Rh-negative candidates a worse result than otherwise-identical Rh-positive ones. US solid-organ allocation is **ABO-matched**; RhD is a red-cell antigen and is not part of OPTN matching for any of the six organs. There is no allocation mechanism that produces the effect the model applies.
- **Measured cost, changing only Rh:** kidney median wait **35.62 → 38.36 months (+2.74)** and p24 **0.7270 → 0.7087**; pancreas +2.28 months; the four other organs between +0.07 and +0.58 months. The suitability score drops 0.70–1.50 points at every center. Per L-084/L-085 the *ranking* is unchanged — blood type reaches only the center-invariant sub-score — so the entire harm is in the magnitudes, not in which center is recommended.
- **The numbers are visibly hand-set.** In multiplier space the Rh adjustment is a flat additive constant per organ: +0.10 for kidney and pancreas, +0.05 for the rest, in **22 of 24 cells**. The two exceptions (lung B +0.03, pancreas A +0.05) read as slips. A data-derived quantity does not come out as the same round number 22 times. The apparent variation *across* ABO groups (AB +18.2% vs O +7.7% for kidney) is an artifact of dividing a constant by different baselines.
- **There are two inconsistent conventions, not one.** SCORE-03 applies a separate Rh penalty of −15 (O), −7 (A), −8 (B), −8 (AB). Nothing reconciles −15 for O with −7 for A, and nothing reconciles either with the flat +0.05/+0.10 in the wait table.
- **No calibration gate can settle it, even in principle.** SRTR publishes blood type as O/A/B/AB **without Rh** — the register already records this twice (DATA-46, EQSP-32, which have to split each group 84/16 by the US Rh share for exactly this reason). So an 8-entry table rests on 4 categories of evidence. The Rh half cannot be calibrated, validated, or refuted by data the project has or can get; it has to be judged on mechanism.
- **Effect on the equity audit:** Rh accounts for 1.4–5.6% of the blood-type variance in the wait multiplier. Small, but a *directional* penalty on ~15% of candidates rather than symmetric noise, so it biases the reported disparity one way.
- **How it was closed (#413):** one canonicalization point rather than six edited tables. `model_key()` maps `X-` to `X+`, and the five lookups (wait multipliers, compatibility score, the explain view, the BBN CPT axis, the MCMC fitted-effect index) call it. The BBN axis deliberately keeps 8 levels — its shape is part of the fitted network — so the collapse shows up in the CPT *values*. All eight UI options remain, and `/score?explain=true` now says outright that Rh does not affect allocation instead of showing a table that implies it does.
- **Guarding against the opposite failure:** making Rh inert by making blood type inert would satisfy half the tests and destroy the model, so every assertion is paired — `O+ == O-` **and** `O != AB`, at the multiplier, score, Monte Carlo and BBN levels (`backend/tests/test_rh_is_inert.py`). Negative-tested by reverting the lookup, by over-collapsing `model_key` to a single group (25 and 12 failures respectively), and by making the explain view drift from the score it explains.
- **Files:** `data/wait-time-distributions.json` (`blood_type_multipliers`), `backend/services/scoring.py:91-95`, `backend/services/scoring_explain.py:69-74`, `backend/services/distributions.py:200`, `backend/services/bbn_parameterizer.py` (`BLOOD_TYPES`), `backend/services/mcmc_inference.py:285`, `scripts/parse-srtr-reports.py:1011-1016`
