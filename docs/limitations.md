...
### L-088: Rh-negative candidates are penalized by the model, and organ allocation does not use Rh
- **Severity:** HIGH (a systematic, directional error in a number patients read, affecting ~15% of them)
- **Status:** DOCUMENTED, NOT FIXED (measured 2026-08-27 from #180 — see `docs/rh-factor-report.md`)
...
- **How to close:** canonicalize blood type to its ABO group once at the model boundary, so the wait multipliers, compatibility score, BBN `BLOOD_TYPES` CPT axis, MCMC index and equity matrix all become Rh-blind together, instead of editing eight-entry tables in six places. Keep all eight UI options — patients know their type, and stating plainly that Rh does not affect allocation is more useful than asking and silently penalizing the answer.
