...
**Rank-materiality evidence (2026-08 sweep, #294):** `scripts/run-assumption-sweep.py` perturbed the major hand-set values ±20% and measured center-rank stability (`docs/assumption-sweep-report.md`). **All swept knobs pass the ρ>0.9 gate** — worst cases: clamp-bound wait factors DATA-24 (ρ 0.973), SCORE-01 waitTime weight (ρ 0.985), log_sigma SURV-13/#274 (ρ 0.993). Patient-level multiplier tables (DATA-01→05) are center-invariant, ρ = 1.0 structurally: their magnitudes shift absolute probabilities (checked instead by calibration + the temporal forecast), not rankings. Not yet swept: copula θ (#255), SUPPLY_WAIT_ELASTICITY, BBN CPT internals (#213/#214), EQSP spatial/equity constants, DATA-20/21 acceptance composite.

**Summary:** **129** assumptions tracked after dedupe. **39** still need justification (`uncited` + `assumed` + `heuristic_clamp`) — SCORE-22 and EQSP-18 moved to `data_derived` with the 2026-08 BEA RPP upgrade (#205), DATA-37 (Puerto Rico placeholder) added as `assumed`; EQSP-32 (`literature`) and EQSP-31 (`assumed`) added with the #254 weighted-Gini fix. **45** are flagged **high-risk** (`risk_if_wrong = high`); the high-risk entries that are also unjustified form the priority shortlist at the end.
...
| SCORE-08 | scoring.py:146-154 | Liver MELD wait multiplier bands (≥35:0.15x … else 2.0x) | Higher MELD = sicker = faster transplant | partially_cited | high | OPTN MELD policy (Phase2, L-002); cutoffs/magnitudes hand-set. #333: input is MELD 3.0 (since 2023, ~1-2 pts higher esp. for women) but band cutoffs were conceived under classic MELD — matters near 14/15, 25/26, 35/36 boundaries |
| SCORE-09 | scoring.py:156-162 | Lung LAS wait multiplier bands (≥50:0.3x … else 1.2x) | Higher LAS = faster transplant | partially_cited | high | OPTN LAS policy (Phase2, L-003); magnitudes hand-set. LAS replaced by CAS in 2023 — CAS input now accepted via the DATA-41 mapping (#303) |
...
| SCORE-15 | scoring.py:240-253 | Living-donor component: per-center SRTR Table D1 scores (living-donor-centers.json), neutral 75 when absent; 28% of donorAvail | Living-donor program strength affects availability | data_derived | low | #292 replaced the dead substring-state-match mechanism (see DATA-40). 28% weight raised from 25% per L-008 |
...
| SCORE-22 | scoring.py:307-326 | Cost-of-living normalization window [80,120]; 40% of geo | Lower COL better | data_derived | low | 2026-08: exact BEA RPP per center MSA (cost_of_living_for_center, #205) replaced 22-city IDW interpolation. Window recalibrated to observed RPP range 83.6–115.6 (2024 vintage). algorithm.js still city-keyed dynamic min/max |
...
| SURV-15 | wait-time-distributions.json:25-91 | cPRA/MELD/LAS clinical wait multiplier bands | Severity scores drive priority/match | partially_cited | high | data-provenance: based on cPRA/MELD/LAS allocation rules; magnitudes hand-set (e.g. cPRA 98-100 → 5x). #333: MELD bands formula-agnostic but boundary-sensitive to the 3.0 shift; LAS retired 2023 (#303) |
...
| SURV-18 | config.py:41-44 | Score drift rates (MELD +2.5/yr; LAS -1.0/yr) | Disease scores worsen over wait | partially_cited | medium | Cites Volk 2006/Bambha 2008 (MELD), "estimates" (LAS) — NONE in paper.bib; cannot verify magnitude tie. #333: drift literature is classic-MELD; unvalidated under MELD 3.0 |
...
| SURV-28 | monte_carlo.py:~190 | Acceptance-rate thinning + national defaults; missing data file → thinning DISABLED (factor 1.0) with warning | Centers accepting fewer offers → longer effective wait | data_derived | medium | acceptance-rates-centers.json composite 0.6·vol+0.4·util, [0.3,3.0]. #219: the old 0.25 code default silently quadrupled every modeled wait when the file was absent |
...
| EQSP-18 | policy_scenarios.py:365-385 | Per-city cost-of-living index (BEA RPP of city MSA, US=100) | Financial-accessibility proxy in travel-subsidy scenario | data_derived | low | 2026-08 (#205): reads live from cost-of-living.json cities block (single source of truth); _CITY_COL_FALLBACK mirrors 2024 snapshot. Replaced stale BLS/fixed-ratio copy (old Indianapolis=74 vs measured 96) |
...
| EQSP-24 | spatial_interpolation.py:21-44 | Hardcoded 22-city coordinate table (fallback nodes) | Spatial support of interpolated surfaces | data_derived | medium | L-063: sparse city data; thin-plate spline edge artifacts. Choice of 22 cities is a coverage assumption. #252 fixed 2026-08-26: the center-bound OUTCOME layers (wait/mortality/graft) were retired from interpolation — administrative values are not continuous fields |
...
| DATA-35 | cost-of-living.json | BEA Regional Price Parities: 387 MSAs + 51 states + US-nonmetro (2024 vintage, US=100) | Area price levels for affordability comparisons | cited | low | BEA MARPP/SARPP bulk files (keyless), fetched by scripts/fetch-cost-of-living.js with dead-data guards (never overwrites snapshot with fewer/implausible values). Replaced 22-city BLS CPI-U blend with 7 hand-estimated cities (#205, #234) |
| DATA-36 | center-cbsa-map.json | Center → CBSA assignment (243 metro, 4 micro, 1 non-CBSA) | Locates each center's metro area for exact RPP lookup | cited | low | Census geocoder point-in-polygon on center lat/lon (scripts/build-center-cbsa-map.py, one-time). Micro/non-CBSA centers fall back to state RPP |
| DATA-37 | data_loader.py cost_of_living_for_center | Puerto Rico centers (PRCC, PRSJ) get US-nonmetro RPP (88.7) | BEA RPP covers neither PR metros nor the territory | assumed | medium | No official RPP exists for PR; nonmetro-US is a documented placeholder. External sources suggest PR prices near/below US average, direction consistent |
...
| EQSP-32 | equity.py BLOOD_TYPE_PREVALENCE | US blood-type prevalence weights (O+ 37.4% … AB- 0.6%) for weighted equity Gini (#254) — population weighting of the 48 demographic cells | literature | low | American Red Cross / Stanford Blood Center US distribution; sums to 1.000 |
...
| DATA-38 | climate-scores-centers.json | Per-center climate recovery scores: NASA POWER climatology features calibrated to the 22 hand-curated scores (R²=0.82, ρ=0.91) — recovery-climate input to geographic score | data_derived | low | #289: replaces the uncited 22-city manual file; formula + fit documented in _meta; comfort anchor 18°C is the one hand-set constant |
| DATA-39 | trauma-scores-centers.json | Per-center trauma scores: FARS 2023 state fatality rates per 100k (Census 2023 pop), max-normalized — traffic-trauma/donor-pool proxy | data_derived | low | #290: replaces 22-city traumaScores; state-level resolution (county refinement tracked in _meta) |
| DATA-40 | living-donor-centers.json | Per-center living-donor scores: SRTR Table D1 LD_CT_1C, log-normalized within organ (kidney/liver) — 28% of donorAvailability | data_derived | low | #292: replaces the hand-curated 22-city livingDonorProgramStrength, which was ALSO dead at runtime (city-in-state substring match never hit → constant 75). log1p scaling hand-chosen |
| DATA-41 | distributions.py cas_to_effective_las | CAS→effective-LAS quantile mapping (29→35, 31→43, 36→60), piecewise linear, clamped [0,100] | Lung urgency input since March 2023 rides the LAS-era multiplier tables | data_derived | medium | #303 option 1: anchors quantile-match the AT-TRANSPLANT score distributions in OPTN/SRTR ADR 2023 Table LU 7 (LAS 2018 vs CAS 2023 cohorts). Approximation — equates urgency percentiles among transplanted candidates; re-derive if OPTN publishes a joint LAS/CAS crosswalk |
| SURV-38 | bayesian_network.py _combine_outcomes + competing_risks.get_patient_mortality_multiplier | Option-B hazard-proportional modulation: patient death hazard = center-observed death hazard × (age × urgency × MELD multiplier); transplant/delisting hazards unmodulated | Patient factors shift the competing-risk split, not just wait timing (#238, closes most of L-072) | partially_cited | medium | Proportional-hazards assumption on the cause-specific death hazard; multipliers from SRTR ADR Table 5.3 (age, restored 2026-08-25 after the #104 deletion) + literature urgency/MELD (SURV-06). Reference patient anchors bit-exactly to the observed vector |
| SURV-39 | rank_stability.py | Rank bootstrap: Beta(p·n+1,(1−p)·n+1) noise at each center's OBSERVED cohort n; centers with no observed record get effective n=8 (deliberately wide) | Rank intervals reflect data-sampling uncertainty only | partially_cited | medium | #313: binomial-SE philosophy shared with the BBN #226 interval; fallback n=8 hand-set to be honest-wide; model-form/parameter uncertainty assessed separately (sweep, MCMC) |
| SURV-40 | monte_carlo.py / stats_utils truncated_wait_times | Accrued-time conditioning: T_remaining ~ (T_eff − t0 | T_eff > t0) on the SAME lognormal; competing-risk clocks restart (memoryless) | Time already served changes remaining-wait estimates (#329) | data_derived | medium | Correct conditional of the existing model, no new parameters. Two encoded assumptions: (1) memoryless mortality/delisting restart at t0 (consistent with SURV-05's exponential hazards); (2) the inspection paradox is surfaced, not suppressed — long time served can raise the remaining median under heavy-tailed lognormals |
| SURV-41 | multi_listing.py | Multi-listing joint model: Gaussian copula over per-listing wait lognormals, pairwise correlation = 250nm-circle overlap fraction; one patient-level competing clock (center-averaged hazards) | Joint P(transplant) for 2-5 listings (#321, closes L-074) | partially_cited | medium | Mechanism verified vs OPTN policy (one match run, per-listing proximity scoring). Overlap correlation is a LOWER bound on true coupling (national sharing) -> joint gain is an upper-bound estimate; Gaussian copula shape assumed; structural bounds max(p_i) <= p_joint <= 1-prod(1-p_i) hold by construction and are test-pinned |
...
| MCMC-34 | mcmc_survival.py EMPIRICAL_FRAC_SIGNAL | Per-organ, PER-METRIC empirical Beta priors on the signal fraction (wait raw 0.63-0.86; mort 0.10-0.33; delist 0.11-0.32; concentration 10; means clamped [0.1,0.9]) | How much cross-center spread is real signal vs release noise | data_derived | low | #317: the replicate observations MCMC-09 called for — unbalanced random-effects ANOVA on the ~13-release center panel (scripts/run-panel-variance.py, docs/panel-variance-report.md, bootstrap CIs). Replaced the symmetric Beta(2,2), which under-trusted wait signal and over-trusted center death rates. Raw (drift-as-noise) estimates chosen as the conservative bound; intestine wait panel insufficient → Beta(2,2) retained |
