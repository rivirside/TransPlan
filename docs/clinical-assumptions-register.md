...
| MCMC-28 | mcmc_survival.py sigma_total_*/frac_signal_* | Beta(2,2) prior on the signal-vs-noise variance fraction (wait/mort/delist) | How much cross-center spread is real signal vs measurement noise | assumed | medium | #207: with one aggregate obs per center only the TOTAL variance is identified — the split is prior-driven by construction (MCMC-09); Beta(2,2) makes that explicit and symmetric. Refine only with replicate observations (multi-release fits) |
