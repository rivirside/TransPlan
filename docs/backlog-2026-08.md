...
| F11 | CPT-parameter MC credible intervals | #296 | partial — Dirichlet CPT sampling MEASURED and rejected: the posterior is narrower than the shipped binomial band (kidney 0.95x, lung 0.59x), so it would have REDUCED disclosed uncertainty. Found and fixed the real defect instead: the Monte Carlo CI was simulation error, identical for n=2 and n=833 centers, shrinking with iterations (L-093). #296 stays open for WaitCategory-timing uncertainty, still unpropagated by either engine |
...
| F14 | CPT empirical grounding + citations | #214 / #233 | partial — register already covers all 20 BBN CPT rows with provenance; the actionable remainder was the constants neither cited NOR measured. BBN-19 (12→24mo extension) now measured: it CANNOT affect p24 (algebraic cancellation) but is the sole determinant of the mortality/removal split, a 4.3x lever, and #297 suggests the shipped value overstates it (L-095). #233 stays open for BBN-02/-12/-14/-15 |
