...
| F17 | L-064 allocation-circle competition proxy | #299 | partial — proxy VALIDATED against observed SRTR rates and found not to predict them; "downgrade its weight" inapplicable (it has none, scoring never imports it). 2026-08-28: the recommended upgrade (candidates-in-circle from TPC_ALL_NC) was tested too and is NO BETTER than the center count — 12 tests, none significant. Unit is not the problem; geometry is the suspect. Disclosure shipped at the numbers + a scope test. #299 stays open with the candidate avenue closed |
